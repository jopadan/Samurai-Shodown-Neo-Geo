...
ModulePlayer::ModulePlayer()
{
	...
	block.PushBack({ 21, 805, 72, 89 }, 0.1, 0, 0, 0, 0);
	block.PushBack({ 93, 804, 75, 89 }, 0.1, 0, 0, 0, 0);
	block.loop = false;
	...
	damage1.PushBack({ 600, 1979, 18, 18 }, 0.4, 0, 0, 0, 0);
	damage1.PushBack({ 623, 1979, 18, 18 }, 0.4, 0, 0, 0, 0);
	damage1.PushBack({ 643, 1978, 20, 20 }, 0.4, 0, 0, 0, 0);
	damage1.PushBack({ 665, 1979, 18, 18 }, 0.4, 0, 0, 0, 0);
	damage1.PushBack({ 0,0,0,0 }, 0.0001, 0, 0, 0, 0);
...
bool ModulePlayer::Start()
{
	...
	//graphics2 = App->textures->Load(DATADIR "Assets/Image/Nakoruru spritesheet 2.png");
	//graphics3 = App->textures->Load(DATADIR "Assets/Image/Nakoruru spritesheet 3.png");
	graphicsobj = App->textures->Load(DATADIR "Assets/Image/objectes.png");
	Kamui = App->music->LoadChunk(DATADIR "Assets/Sound/Nakoruru/Samurai Shodown - Nakoruru - Kamui Mutsube.wav");
	swordlight = App->music->LoadChunk(DATADIR "Assets/Sound/Common/Samurai Shodown - A- 01.wav");
	swordheavy = App->music->LoadChunk(DATADIR "Assets/Sound/Common/Samurai Shodown - A (LONG)- 01.wav");
	kickslight = App->music->LoadChunk(DATADIR "Assets/Sound/Common/Samurai Shodown - KICK (MISS) - 01.wav");
	kicksheavy = App->music->LoadChunk(DATADIR "Assets/Sound/Common/Samurai Shodown - KICK (MISS) - 02.wav");
    hitted = App->music->LoadChunk(DATADIR "Assets/Sound/Nakoruru/Samurai Shodown - Nakoruru - Hitted 2.wav");
	heavydo= App->music->LoadChunk(DATADIR "Assets/Sound/Nakoruru/Samurai Shodown - Nakoruru - 02.wav");
	heavyya = App->music->LoadChunk(DATADIR "Assets/Sound/Nakoruru/Samurai Shodown - Nakoruru - 01.wav");
	annu = App->music->LoadChunk(DATADIR "Assets/Sound/Nakoruru/Samurai Shodown - Nakoruru - Annu Mutsube.wav");
	protection= App->music->LoadChunk(DATADIR "Assets/Sound/Common/Samurai Shodown - ATTACK PROTECTION 03.wav");
...
update_status ModulePlayer::Update()
{
	...
	if (state != current_state)
	{
			...
			if (animstart == 0 || 2)
			{
				...
				case 1:
					...
					if (animstart == 0) {
						...
						if (collider == true) {
							colliderAttack = App->collision->AddCollider({ position.x, position.y , 50, 30 }, COLLIDER_PLAYER_SHOT, this);
				...
				case 2:
					...
					if (animstart == 0) {
						...
						if (collider == true) {
							colliderAttack = App->collision->AddCollider({ position.x, position.y , 60, 30 }, COLLIDER_PLAYER_SHOT, this);
				...
				case 3:
					if (animstart == 0) {
						...
						if (collider == true) {
							colliderAttack = App->collision->AddCollider({ position.x, position.y , 30, 30 }, COLLIDER_PLAYER_SHOT, this);
			...
			if (animstart == 0 || 2)
			{
				...
				case 2:
					...
					if (animstart == 0) {
						...
						if (collider == true) {
							colliderAttack = App->collision->AddCollider({ position.x, position.y , 50, 50 }, COLLIDER_PLAYER_SHOT, this);
							App->music->PlayChunk(swordheavy);
							collider = false;
							time = SDL_GetTicks();
						}
		...
		case ST_HEAVY_PUNCH_STANDING:
			...
			if (collider == true) {
				colliderAttack = App->collision->AddCollider({ position.x, position.y , 45, 80 }, COLLIDER_PLAYER_SHOT, this);
				App->music->PlayChunk(swordheavy);
				App->music->PlayChunk(heavydo);
				collider = false;
				time = SDL_GetTicks();
		...
		case ST_LEYLA_MUTSUBE:
			...
			if (collider == true) {
				colliderAttack = App->collision->AddCollider({ position.x, position.y, 50, 20 }, COLLIDER_PLAYER_SHOT, this);
		...
		case ST_YATORO_POKU:
			...
			if (collider == true) {
				colliderAttack = App->collision->AddCollider({ position.x, position.y, 40, 45 }, COLLIDER_PLAYER_SHOT, this);
				App->music->PlayChunk(swordheavy);
				App->music->PlayChunk(heavydo);
				collider = false;
				time = SDL_GetTicks();
...
//	if (App->ui->damage_p1 <= 26) {
	...
	if (playdamage == true) {
		if (flip == 0) {
			...
			App->render->Blit(graphics, position.x + 20, position.y - 70, &(damage1.GetCurrentFrame()), SDL_FLIP_NONE);
		}
		if (flip == 1) {
			App->render->Blit(graphics, position.x + 3, position.y - 50, &(damage1.GetCurrentFrame()), SDL_FLIP_NONE);
			App->render->Blit(graphics, position.x + 13, position.y - 70, &(damage1.GetCurrentFrame()), SDL_FLIP_NONE);
		}
	}

	if (flip == SDL_FLIP_HORIZONTAL) {
		if (colliderPlayer != NULL)colliderPlayer->SetPos(position.x, position.y - 80 + height);
	}
	else {
		if (colliderPlayer != NULL)colliderPlayer->SetPos(position.x + 9, position.y - 80 + height);
	}
	if (flip == SDL_FLIP_HORIZONTAL) {
		if (colliderPlayer_2 != NULL)colliderPlayer_2->SetPos(position.x + 7, position.y - 50 - height2);
	}
	else {
		if (colliderPlayer_2 != NULL)colliderPlayer_2->SetPos(position.x + 15, position.y - 50 - height2);
	}
...
player_states ModulePlayer::process_fsm(p2Qeue<player_inputs>& inputs) {
	...
	while (inputs.Pop(last_input))
	{
		...
		case ST_PUNCH_STANDING:
		{
			switch (last_input)
			{
			case IN_2: if (SDL_GetTicks() - Heavytimer < 50) { state = ST_HEAVY_PUNCH_STANDING; App->input->heavy_punch_timer = SDL_GetTicks(); } break;
		...
		case ST_MEDIUM_PUNCH_STANDING:
		{
			switch (last_input)
			{
			case IN_1: if (SDL_GetTicks() - Heavytimer < 50) { state = ST_HEAVY_PUNCH_STANDING; App->input->heavy_punch_timer = SDL_GetTicks();  } break;
			case IN_PUNCH_FINISH: state = ST_IDLE; animstart = 0; break;
			case IN_DAMAGE: state = ST_DAMAGE; animstart = 0;  break;
			case IN_WIN: state = ST_WIN; break;
			case IN_DEFEAT: state = ST_DEFEAT; break;
		...
		case ST_KICK_STANDING:
		{
			...
			switch (last_input)
			{
			case IN_4: if (SDL_GetTicks() - Heavytimer < 50) { state = ST_HEAVY_KICK_STANDING; App->input->heavy_kick_timer = SDL_GetTicks();  }break;
		...
		case ST_MEDIUM_KICK_STANDING: {
			...
			switch (last_input)
			{
			case IN_3:if (SDL_GetTicks() - Heavytimer < 50) { state = ST_HEAVY_KICK_STANDING; App->input->heavy_kick_timer = SDL_GetTicks();  } break;
			case IN_KICK_FINISH: state = ST_IDLE; animstart = 0;  break;
			case IN_DAMAGE: state = ST_DAMAGE; animstart = 0;  break;
			case IN_WIN: state = ST_WIN; break;
			case IN_DEFEAT: state = ST_DEFEAT; break;
		...
		case ST_CROUCH:
		{
			...
			case IN_2:
				...
				if (combo2 == 3) { mutsubespeed = 5; state = ST_ANNU_MUTSUBE; App->input->AnnuM_timer = SDL_GetTicks(); combo2 = 0; break; }
					...
					state = ST_HEAVY_PUNCH_CROUCH; App->input->heavy_punch_timer = SDL_GetTicks(); combo2 = 0; animstart = 0; break;
				}
				break;
			case IN_3:
				...
				break;
			case IN_4:
				state = ST_HEAVY_KICK_CROUCH; App->input->heavy_kick_timer = SDL_GetTicks(); combo2 = 0; animstart = 0;
				break;
			case IN_DAMAGE: state = ST_DAMAGE; animstart = 0;  break;
			case IN_WIN: state = ST_WIN; break;
			case IN_DEFEAT: state = ST_DEFEAT; break;
		...
		case ST_PUNCH_CROUCH:
			...
			switch (last_input)
			{
			case IN_PUNCH_CROUCH_FINISH: state = ST_IDLE; animstart = 0;break;
			case IN_DAMAGE: state = ST_DAMAGE; animstart = 0;  break;
			case IN_WIN: state = ST_WIN; break;
			case IN_DEFEAT: state = ST_DEFEAT; break;
			}
		...
		case ST_HEAVY_PUNCH_CROUCH:
			...
			switch (last_input)
			{
			case IN_PUNCH_FINISH: state = ST_CROUCH; animstart = 0; break;
			case IN_DAMAGE: state = ST_DAMAGE; animstart = 0;  break;
			case IN_WIN: state = ST_WIN; break;
			case IN_DEFEAT: state = ST_DEFEAT; break;
			}
...
void ModulePlayer::OnCollision(Collider* c1, Collider* c2) {
	...
	if (colliderPlayer == c1 && c2->type == COLLIDER_ENEMY_SHOT && defense == false)
	{
		...
		if (colliderPlayer == c1 && c2->type == COLLIDER_ENEMY_SHOT && defense == true) App->input->inputs.Push(IN_BLOCK); if (App->player2->colliderAttack != NULL) App->player2->colliderAttack->to_delete = true;
